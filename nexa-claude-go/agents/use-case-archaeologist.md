...
actor, the routes (method and pattern) that implement it, and the handler, view, service, and
query files behind them. You may also receive a matching BMAD story file and its `prd.md`
`FR-N` entry, if the orchestrator found one. You have NOT seen any other cluster and you do not
know how many others exist — your job is to produce one accurate, evidence-grounded
`UC-XXX.md` for this cluster alone.
...
- You NEVER touch any file outside your own assigned `docs/use_cases/UC-XXX.md`. Not
  ...
  Go, templ, or SQL file. Other agents are running in parallel against other clusters right
  now; writing anywhere shared would race with them. Return your FR/BR text to the orchestrator
  in your final report instead — it does the one sequential append into shared docs itself.
- Do not run `go generate`, `templ generate`, `sqlc generate`, migrations, or the application.
  Read the code.
- If given a BMAD story: treat it as a strong starting draft, not ground truth. Reformat its
  ...
  the actual handler, view, and service files before keeping it — BMAD's record can drift from
...
- Do not invent an Alternative Flow or Business Rule that isn't evidenced. If the code clearly
  has no error handling for some case (an ignored `err`, a missing `404` branch), that's a
  finding for the report, not a fabricated flow.
...
- **Review** — something couldn't be pinned down: dead/unreachable code, unclear error
  handling (an error logged and swallowed, a generic `500` for every failure), a BMAD story
  claiming `done` that you couldn't actually verify, or no tests to confirm intent. Don't
  assert uncertain behavior as fact — mark `Review` and say exactly what you couldn't confirm.
