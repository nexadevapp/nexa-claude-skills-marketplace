...
See [REFERENCE.md](REFERENCE.md#traceability-convention-details) for: multiple-UC files,
pure bug regression test files, the full annotated example, why a raw `test.describe()`
is required instead of a custom wrapper, and the inline-comment convention for CR/BUG-driven
assertions.
...
a **suite-level user** shared by all tests in a file (created in `beforeAll`, standard
active/confirmed user for happy paths), and a **per-test user override** for tests that need
different characteristics (inactive status, unconfirmed email, a different account type —
created before the test, logged in via the UI, cleaned up in a `finally` block). See
[REFERENCE.md](REFERENCE.md#test-user-provisioning-details) for the full field-by-field
lifecycle of both levels.
...
server. This eliminates dynamic env file generation and scattered `env:` blocks. Commit this
file (add `!.env.e2e` to `.gitignore` if `.env*` is ignored). See
[REFERENCE.md](REFERENCE.md#enve2e-contents) for the required variables.

### Playwright Configuration

**Single browser only** — use Chromium. Do NOT add Firefox or WebKit projects. Cross-browser
testing is not the purpose of E2E tests; verifying user journeys is. See
[REFERENCE.md](REFERENCE.md#playwright-configuration-example) for a full `playwright.config.ts`
example wired to the global setup/teardown described above.
...
Code patterns for suite-level and per-test user provisioning, login via the UI, navigating
only through UI interactions, verifying intermediate and final state, form interactions,
entity cleanup, and dialog handling: see
[REFERENCE.md](REFERENCE.md#common-patterns-details).

## Assertions Reference

See [REFERENCE.md](REFERENCE.md#assertions-reference-table) for the standard Playwright
assertion forms (text content, input value, element count, visibility, URL, enabled state,
partial text).
