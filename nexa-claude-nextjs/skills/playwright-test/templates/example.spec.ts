...
import { useCase, bugTest } from './helpers/traced';
...
// 4. `useCase()` and `bugTest()` from `./helpers/traced` are the only
//    allowed test-registration forms. Top-level `test()` and `test.describe()`
//    are prohibited.

// ── Suite-level user (shared across all tests in this file) ─────────────
//
// `test.beforeAll` / `test.afterEach` / `test.afterAll` are framework hooks,
// not test definitions — that's why they're allowed on the imported `test`.
...
//
// Hoisted to module scope so both `useCase()` tests AND `bugTest()` calls
// can use them. The helper performs the login form flow without asserting
// success, so it works for both happy paths and failure paths (e.g. AF-2's
// suspended-user assertion).
...
useCase('UC-001', 'Manage Items', (test) => {
...
//
// Use bugTest() when a bug has no natural use-case scenario to attach to —
...
bugTest('BUG-002', 'login does not crash on Unicode characters in email', async ({ page }) => {
