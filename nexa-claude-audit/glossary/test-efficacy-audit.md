...
|------|---------|------------------------------|
| Test efficacy | How well the tests detect a change in the behaviour of the code. | Coverage shows that a line ran. The mutation score shows that a test checks the result of the line. |
...
| Status | `pass`: the value is within the threshold. `warn`: the value is near the limit. `fail`: the value is outside the limit. | Line coverage and mutation score each have a status. |
