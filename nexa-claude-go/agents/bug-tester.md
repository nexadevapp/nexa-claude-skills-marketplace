...
- If the dev server is not already running, start it (`go run ./cmd/dev`) before attempting any
  browser-based step. It starts a Postgres Testcontainer, so Docker must be running — if it is
  not, stop and report that instead of guessing a verdict.
