...
description: >
  Orchestrates the full per-use-case delivery pipeline for the React SPA + ASP.NET Core stack:
  verifies spec and design exist, implements the use case, writes E2E tests, and evaluates
  coverage against the spec. Iterates automatically until quality gates pass. Specification and
  design must exist beforehand via /sprint-prepare, /use-case-spec, or /design-screens.
...
Invoke to deliver a single use case end to end for the React SPA + ASP.NET Core stack. Run it
only when its specification and design already exist. Explicit-invoke only: trigger via
`/deliver-use-case UC-XXX` or from `/sprint-deliver` — never infer it from a conversational
request.

> Note: the `/sprint-deliver` and `/sprint-kickoff` skills are planned for this stack but are
> not yet part of it. This skill runs standalone via `/deliver-use-case` until they land.
...
- Create or modify entities in `docs/entity_model.md` or the EF Core model (entity classes / DbContext) during the pipeline — all entities must exist before this skill is invoked
...
2. Each entity exists in the EF Core model (an entity class registered on the `DbContext`)

If any entity is missing from either place, stop immediately:

```
PIPELINE STOPPED: Missing entities for $ARGUMENTS

Missing from docs/entity_model.md: [entity names]
Missing from the EF Core model (entity classes / DbContext): [entity names]

Run /entity-model to update the entity model, then /ef-migration to create the
...
**Verify:**
1. Backend `dotnet build` succeeds
2. Backend `dotnet test` (unit tests) passes
...
Do not proceed until backend build, backend unit tests, and frontend build all pass and no
Critical DoD items are outstanding.
...
Invoke via the Agent tool with `subagent_type: "nexa-claude-react-spa-dotnet:playwright-test"`. Prompt:
...
- **Implementation bug** — wrong behavior, missing route, server error, data mismatch.
  Indicators: wrong HTTP status, missing API endpoint, UI renders wrong content.

Log the iteration to the delivery log (see Delivery Log), then act:

1. **Test bugs:** Re-launch the `nexa-claude-react-spa-dotnet:playwright-test` subagent with
   the error context and iteration history. Return to Phase 2.
2. **Implementation bugs:** Fix in main context. Re-run `dotnet build`, `dotnet test`, and
   `npm run build` to confirm the fix doesn't break anything. Return to Phase 2.
3. **Mixed:** Fix implementation bugs first, then re-launch the
   `nexa-claude-react-spa-dotnet:playwright-test` subagent for test bugs.
...
If there are **Missing** items, re-launch the `nexa-claude-react-spa-dotnet:playwright-test`
subagent with the gap analysis and iteration history as input. After it returns, independently
verify tests pass (same as Step 3 Phase 2).
...
Include a **What was built** section listing key artifacts (React pages/components, API
controllers/endpoints, services, tests) and links to `docs/delivery/$ARGUMENTS-iterations.md`
and `docs/delivery/$ARGUMENTS-traceability.md`.
...
- Entity Gate passed: every UC entity exists in `docs/entity_model.md` and the EF Core model.
- Step 2 gates green: `dotnet build`, `dotnet test`, and `npm run build` all pass, no Critical DoD items outstanding.
- Step 3: `npx playwright test` run by the main context shows 0 failed, 0 skipped, exit code 0.
- Step 4: coverage evaluation reports PASS with no Missing items on critical FRs/UC steps.
- `docs/use_cases/$ARGUMENTS.md` Status updated to `Done`.
- `docs/delivery/$ARGUMENTS-traceability.md` written and the summary posted to the GitHub issue (or user informed none exists).
- On any exhausted loop, Failure Recovery was offered and the rollback commit is available.
