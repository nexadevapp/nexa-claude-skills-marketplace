...
## 8 · Automated capture script

All 17 snapshots (sections 01–08) are generated by a single script:

```bash
node tools/snap-all.mjs
```

**Prerequisites:** `bun install` and `npx playwright install chromium`.

The script uses Playwright headless Chromium at 2x DPR / 1440px viewport. It:

- Renders `requirements.md` and `UC-002.md` as styled dark-theme HTML (matching the shore palette)
- Renders the Mermaid ER diagram via mermaid.live CDN
- Renders the PlantUML use case diagram via plantuml.com (hex encoding)
- Captures full-page wireframe + individual screen cards by element ID
- Captures individual design states by `section.mb-10` index (skipping Overview and Data mapping sections)

The script is idempotent — it overwrites existing files in `docs/snapshots/`. After running, compress any file > 2 MB with `pngquant --quality=80-95 <file> --output <file> --force`.

## 9 · Re-capturing when the artifacts change

Re-run `node tools/snap-all.mjs` after any change to the source artifacts. The PDF must reflect the current state of the artifacts, not an older revision.

## 10 · Adding new snapshots

To add a snapshot for a new artifact:

1. Add the capture logic to `tools/snap-all.mjs` (follow the existing patterns for HTML, Markdown, or diagram sources).
2. Add the filename to the naming convention list in §1 of this document.
3. Add a per-artifact subsection to §4 with the **Source**, **Goal**, **Capture method**, and **Must include** fields.
4. Run `node tools/snap-all.mjs` and verify the output.
5. Update `REVIEW.md` with the new entry in the inventory table.
