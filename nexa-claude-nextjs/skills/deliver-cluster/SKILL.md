...
| File | Rule |
|------|------|
| `prisma/schema.prisma` | Never touched during delivery — the Entity Gate in `/deliver-use-case` already forbids it. If a use case in the ready set needs a schema change, stop. The migration must land on `main` before the cluster starts, and it runs in its own worktree because `/prisma-migration` is gated too: `git worktree add ../<repo>-schema -b tt/TT-XXX origin/main`, run `/prisma-migration` there, then `/merge-use-case TT-XXX` |
| `messages/*.json` | Warn when two ready use cases add keys to the same namespace. The merge gate resolves it, but a per-use-case namespace file avoids the conflict entirely |
| The request interception entry point | Warn when two ready use cases both add route rules |

A warning does not stop the run. A `prisma/schema.prisma` change does.
