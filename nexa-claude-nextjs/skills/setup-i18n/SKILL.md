...
Each file starts with a minimal structure containing common namespaces (`common`,
`navigation`, `validation`, `auth`) — see
[REFERENCE.md](REFERENCE.md#step-4-default-locale-message-file) for the full starter JSON.
...
see [REFERENCE.md](REFERENCE.md#step-4-non-default-locale-placeholder-example) for an example.
...
standard `TT-XXX` naming convention: read `docs/technical_tasks/` to determine the next
available `TT-XXX` ID, then create `docs/technical_tasks/TT-XXX-i18n-retrofit.md` from
`nexa-claude-core/skills/technical-task/templates/technical-task.md`. See
[REFERENCE.md](REFERENCE.md#step-10b-i18n-retrofit-task-fields) for the exact field values
(Task Name, Category, Goal, Status, Acceptance Criteria grouping, Affected Areas).
...
3. Append the section from [REFERENCE.md](REFERENCE.md#step-13-claudemd-section-template)
...
[REFERENCE.md](REFERENCE.md#step-14-summary-template): strategy, files created/updated,
middleware integration, test results, retrofit status (if applicable), how to use in feature
