...
//
// 1. ONE `page.goto(...)` per test — at the entry point only (the SPA on
//    :3000). All subsequent navigation must happen through UI interactions.
// 2. `page.request.get/post/delete(...)` is API I/O, not navigation. In this
//    split stack the API is a DIFFERENT origin (:5000), so API I/O uses
//    absolute URLs against `API` — never the SPA baseURL.
// 3. Wait for specific UI elements (heading, table row, form field). Never use
//    `page.waitForLoadState('networkidle')` — the CRA dev server's HMR
//    websocket keeps the network busy and it never settles.
...
// API base — the ASP.NET Core origin. API I/O in tests targets this, not the SPA.
...
test.afterEach(async ({ context }) => {
  // Clear cookies + storage between tests to prevent session leakage
...
test.describe('UC-001: Manage Items', uc('UC-001'), () => {
  ...
  test('user logs in, navigates to items, creates item, and sees it in the list',
    ...
    async ({ page }) => {
      ...
      // Cleanup via API (not navigation — page.request is I/O, hits the API on :5000).
      const response = await page.request.get(`${API}/api/examples?name=E2E+Test+Item`);
      const items = await response.json();
      if (items.length > 0) {
        await page.request.delete(`${API}/api/examples/${items[0].id}`);
      }
    });
  ...
  test('user logs in, navigates to items, and sees empty state when no items exist',
    ...
    async ({ page }) => {
      // Login first so subsequent page.request calls share the auth token/cookie.
      await loginViaUI(page, suiteUser);

      // API setup: clear seed data so the empty state is reachable.
      // page.request is API I/O against :5000, not navigation — not a goto.
      const list = await page.request.get(`${API}/api/examples`);
      const allItems = await list.json();
      for (const item of allItems) {
        await page.request.delete(`${API}/api/examples/${item.id}`);
      }
  ...
  test('user creates a categorized item; empty Category is rejected',
    ...
    async ({ page }) => {
      ...
      const response = await page.request.get(`${API}/api/examples?name=CR-Widget`);
      const items = await response.json();
      if (items.length > 0) {
        await page.request.delete(`${API}/api/examples/${items[0].id}`);
      }
...
test('login does not crash on Unicode characters in email',
  ...
  async ({ page }) => {
    ...
    // invalid; we're testing that the API doesn't 500 on Unicode input).
    await page.goto('/login');
    await expect(page.getByLabel('Email')).toBeVisible();
    ...
    // Pre-fix: API returned 500. Post-fix: friendly invalid-credentials.
